...
def compute_metric(data, num_folds,target_column,max_layers,max_units,resources):
    ...
    for folds in tqdm(fold_order):
        ...
        best_model_A = NN(tune_parameters(NN.tune,train_data,target_column,max_layers,max_units,resources)).model
        ...
        best_model_B = NN(tune_parameters(NN.tune,evaluation_data,target_column,max_layers,max_units,resources)).model
        test_data_x = test_data.drop(columns=[target_column], axis=1)
        predicted_difficulty = 1 - best_model_B.predict(test_data_x,verbose=0).reshape(-1)
